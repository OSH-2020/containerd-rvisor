...
func New(ctx context.Context, id string, publisher shim.Publisher, shutdown func()) (shim.Shim, error) {
	ep, err := runrsc.NewOOMEpoller(publisher)
	if err != nil {
		return nil, err
	}
	go ep.Run(ctx)
	s := &service{
		...
		ec:         make(chan runrsc.Exit),
		ep:         ep,
		cancel:     shutdown,
		containers: make(map[string]*runrsc.Container),
	}
	go s.processExits()
...
type service struct {
	...
	containers map[string]*runrsc.Container
...
func (s *service) Cleanup(ctx context.Context) (*taskAPI.DeleteResponse, error) {
	...
	//runtime, err := runrsc.ReadRuntime(path)
	//if err != nil {
	//	return nil, err
	//}
...
func (s *service) Create(ctx context.Context, r *taskAPI.CreateTaskRequest) (_ *taskAPI.CreateTaskResponse, err error) {
	...
	container, err := runrsc.NewContainer(ctx, s.platform, r, s.ec)
	if err != nil {
		return nil, err
	}
...
func (s *service) checkProcesses(e runrsc.Exit) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *service) forward(ctx context.Context, publisher shim.Publisher) {
	...
	for e := range s.events {
		...
		err := publisher.Publish(ctx, runrsc.GetTopic(e), e)
		cancel()
		if err != nil {
...
func (s *service) getContainer(id string) (*runrsc.Container, error) {
	s.mu.Lock()
